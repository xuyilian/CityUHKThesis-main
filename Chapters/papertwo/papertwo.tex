...
cancelling it.  The vehicle therefore sustains a large, persistent yaw rate by design; and because the tangential rotors are commanded independently of the lift channel, that rate is set rather than inherited from the thrust balance.  This sustained rotation is what shapes the dynamics and motivates the
...
$|\alpha_x|, |\alpha_y| \ll 1$, the deviation of the thrust axis $\hat{z}^b$
from vertical is parametrised by the reduced attitude state
\begin{equation}
    \boldsymbol{\eta} = [\alpha_x,\; \alpha_y]^{\top},
    \label{eq:eta_def}
\end{equation}
where $\alpha_x$ and $\alpha_y$ are the small inclinations of $\hat{z}^b$
about the inertial $\hat{x}^w$ and $\hat{y}^w$ axes respectively.  The
non-revolving axes are then approximated as
\begin{equation}
...
driven by a constant yaw term in the setpoint, carried on the yaw channel of
the mixer and therefore applied as a differential between the two rotor pairs;
because that term is issued independently of the lift channel, the spin rate is
...
The symmetry of  that the rotation-induced
...
first-harmonic lift imbalance and hence a torque coupling translation into tilt, the advancing/retreating blade effect familiar from rotorcraft.  This
...
of Chapter~\ref{chapter:paperthree} were obtained without it.  .
...
drive is an independent input, so $\omega_z$ can be selected, and the relevant question becomes what value to select.
...
two-dimensional description in the tilt state $\boldsymbol{\eta}$
of~\eqref{eq:eta_def} that eliminates the fast yaw and retains only the controllable attitude evolution.
...
\end{equation}
The last two terms are the known plant dynamics (aerodynamic damping and gyroscopic coupling) to be cancelled by the control law.
...
means the spin is sustained by construction whatever the lift channel is doing, including when the lift command is removed entirely.  This is what
