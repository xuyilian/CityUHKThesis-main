...
At yaw rate $\omega_z$, each foil at tip radius $r_f$ moves at tangential
speed $\omega_z r_f$.  Its profile drag, acting opposite to the sweep
direction, is
\begin{equation}
    D_{f,i} = k_{D,f}\,(\omega_z r_f)^2,
    \qquad
    k_{D,f} = \tfrac{1}{2}\rho\,C_{D,f}\,S_f,
    \label{eq:foil_drag}
\end{equation}
where $C_{D,f}$ is the hydrofoil profile drag coefficient and $S_f$ is the
foil planform area.  For a vehicle operating at a fixed altitude, $\rho$ is a
constant of the environment; $k_{D,f}$ is therefore determined entirely by the
foil geometry ($C_{D,f}$, $S_f$) and is a fixed design parameter.  Each drag
force acts at moment arm $r_f$ about the $\hat{z}^b$ axis, contributing a
yaw-retarding torque $-k_{D,f}r_f^3\omega_z^2$ per foil.  Extracting the
$\hat{z}^b$ projection of~\eqref{eq:euler_full}, using
...
\begin{equation}
    I_z\dot{\omega}_z
        = c\,F
          - 4\,k_{D,f}\,r_f^3\,\omega_z^2.
    \label{eq:yaw_ode}
\end{equation}
The quadratic drag on the right-hand side is set entirely by the foil
geometry; it creates a unique stable equilibrium at $\omega_z^* > 0$.
At sustained hover, $F = mg$, giving the \emph{relaxed hover spin rate}
\begin{equation}
    \boxed{\omega_z^* = \sqrt{\frac{c\,m\,g}{4\,k_{D,f}\,r_f^3}}.}
    \label{eq:omegaz_hover}
\end{equation}
This equilibrium is reached entirely passively: the foil drag automatically
balances the propeller yaw torque without any active control input.
...
(yaw drive per unit thrust) and the foil drag stiffness $4k_{D,f}r_f^3$
...
The foil tip radius $r_f$ is particularly influential: the drag torque
scales as $r_f^3$ (moment arm $r_f$ multiplied by drag force $\propto
(\omega_z r_f)^2$), so placing the foils further from the centre slows the
spin strongly.  Because the foils are mounted between the motor arms at
radius $r_f > l$, this placement can be adjusted independently of the motor
arm length $l$, providing an additional design lever for tuning $\omega_z^*$.
Increasing the foil planform area $S_f$ or selecting a foil profile with
higher $C_{D,f}$ has the same slowing effect through $k_{D,f}$.
Conversely, increasing the propeller reaction-to-thrust ratio $c$ speeds
the vehicle up.
