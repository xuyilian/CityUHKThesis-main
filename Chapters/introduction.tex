...
most amphibious aerial robots treat water contact as a terminal state (landing, floating, or sampling) rather than as a mode of locomotion
...
foils therefore impose almost no penalty on flight, generating negligible aerodynamic lift as the vehicle hovers and spins, yet deliver a powerful
...
spin rate is commanded rather than passively determined, it can be treated as a design parameter, tuned to whatever tangential blade speed the hop requires.
...
\todobox{citations throughout the thesis}{%
Every \texttt{\textbackslash cite\{??\}} below is unresolved, and the bibliography holds only
three real entries (the two SplitFlyer papers and Mueller \& D\primeAndrea).
Needed: amphibious aerial robots; water take-off and launch mechanisms;
stone-skipping and water-entry physics. The prose does not depend on which
sources are chosen, so this can be completed independently of the writing.}
...
treated as a destination (a dive, a landing, or a sampling stop), and the return to the air is handled either by fully restarting flight or by a
...
sinks~\cite{??}.  These studies characterise the conditions for a successful skip (approach angle, speed, and spin) but consider passive, unpowered
